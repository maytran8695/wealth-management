...
/* ---------------------------------- THEME ---------------------------------- */
const C = {
  bg: "#0C0913", bgPanel: "#161020", bgPanel2: "#1D1629",
  border: "#2C2338", borderLight: "#3A2F4A",
  gold: "#C9A54D", goldBright: "#E8C97A",
  violet: "#8B6FD4", violetLight: "#B39FE8", violetDeep: "#4A3870",
  text: "#F0E9F7", textMuted: "#A395B5", textFaint: "#6E6280",
  danger: "#C4746A", success: "#7FA88C",
};

const TIERS = {
  1: { label: "An toàn", color: "#7FA88C" },
  2: { label: "Ổn định tăng trưởng", color: "#8B6FD4" },
  3: { label: "Tăng trưởng cao", color: "#C9A54D" },
  4: { label: "Đầu cơ", color: "#C4746A" },
};
...
function Card({ children, style }) {
  return <div className="rounded-2xl p-5" style={{ background: C.bgPanel, border: `1px solid ${C.border}`, ...style }}>{children}</div>;
}
function SectionTitle({ eyebrow, title, action }) {
  return (
    <div className="flex items-center justify-between mb-4">
      <div>
        {eyebrow && <div className="text-xs uppercase mb-1" style={{ color: C.gold, letterSpacing: "0.15em" }}>{eyebrow}</div>}
...
function GoldButton({ children, onClick, style, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled} className="flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium"
      style={{ background: disabled ? C.borderLight : `linear-gradient(135deg, ${C.gold}, #A8813A)`, color: disabled ? C.textFaint : "#1A1420", opacity: disabled ? 0.6 : 1, cursor: disabled ? "not-allowed" : "pointer", ...style }}>
      {children}
    </button>
  );
}
...
const inputStyle = { background: C.bg, border: `1px solid ${C.borderLight}`, color: C.text, borderRadius: 10, padding: "9px 12px", fontSize: 14, outline: "none", width: "100%" };
...
export default function WealthCompass() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ borderBottom: `1px solid ${C.border}`, background: "rgba(12,9,19,0.9)", position: "sticky", top: 0, zIndex: 20, backdropFilter: "blur(8px)" }}>
        <div className="max-w-5xl mx-auto px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, fontWeight: 700, lineHeight: 1 }}>La Bàn Gia Sản</div>
              <div style={{ fontSize: 10, color: C.textFaint, letterSpacing: "0.1em" }}>QUẢN LÝ GIA SẢN · DỮ LIỆU MINH HỌA</div>
            </div>
          </div>
          <div className="text-right">
            <div style={{ fontSize: 11, color: C.textMuted }}>Tài sản ròng</div>
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 20, fontWeight: 700, color: C.goldBright }}>{fmtFull(netWorth)}</div>
          </div>
        </div>
        <div className="max-w-5xl mx-auto px-5 flex gap-1 overflow-x-auto pb-2">
          {tabs.map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)} className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm whitespace-nowrap"
              style={{ background: tab === t.id ? C.bgPanel2 : "transparent", color: tab === t.id ? C.goldBright : C.textMuted, border: tab === t.id ? `1px solid ${C.borderLight}` : "1px solid transparent" }}>
...
function AlertsOverviewTab({ alerts, reminders }) {
  return (
    <div className="flex flex-col gap-5">
      <Card>
        <SimpleTitle title="Rủi ro" action={alerts.length > 0 && <span style={{ fontSize: 11, padding: "2px 9px", borderRadius: 99, background: C.danger + "22", color: C.danger }}>{alerts.length}</span>} />
        {alerts.length === 0 ? <EmptyState text="Không có cảnh báo rủi ro nào — mọi chỉ số trong ngưỡng an toàn." /> : (
          <div className="flex flex-col gap-3">
            {alerts.map((a) => (
              <div key={a.id} className="rounded-xl p-4" style={{ background: C.bg, border: `1px solid ${a.level === "danger" ? C.danger : C.gold}55` }}>
                <div className="flex items-center gap-2 mb-1.5"><AlertTriangle size={15} color={a.level === "danger" ? C.danger : C.gold} /><div style={{ fontSize: 14, fontWeight: 600 }}>{a.title}</div></div>
      ...
      <Card>
        <SimpleTitle title="Nhắc nhở" action={reminders.length > 0 && <span style={{ fontSize: 11, padding: "2px 9px", borderRadius: 99, background: C.violet + "22", color: C.violetLight }}>{reminders.length}</span>} />
        {reminders.length === 0 ? <EmptyState text="Không có nhắc nhở nào hiện tại." /> : (
          <div className="flex flex-col gap-3">
            {reminders.map((r) => (
              <div key={r.id} className="rounded-xl p-4" style={{ background: C.bg, border: `1px solid ${C.violet}44` }}>
                <div className="flex items-center gap-2 mb-1.5"><CalendarClock size={15} color={C.violetLight} /><div style={{ fontSize: 14, fontWeight: 600 }}>{r.title}</div></div>
...
/* -------------------------------- DASHBOARD -------------------------------- */
function Dashboard({ totalAssets, totalLiabilities, netWorth, allocation, tierTotals, netWorthHistory, backEndDTI, liquidityRunway, liquidAssets, totalMonthlyDebtService, monthlyIncome, monthlyExpense, netCashflow, goals, alerts, assets }) {
  const total = Object.values(tierTotals).reduce((a, b) => a + b, 0) || 1;
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="grid grid-cols-3 gap-4">
        <Card><div style={{ fontSize: 12, color: C.textMuted }}>Tổng tài sản</div><div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 26, fontWeight: 700, marginTop: 4 }}>{fmt(totalAssets)}</div></Card>
        <Card><div style={{ fontSize: 12, color: C.textMuted }}>Tổng nợ</div><div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 26, fontWeight: 700, color: C.danger, marginTop: 4 }}>{fmt(totalLiabilities)}</div></Card>
        <Card><div style={{ fontSize: 12, color: C.textMuted }}>Tài sản ròng</div><div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 26, fontWeight: 700, color: C.goldBright, marginTop: 4 }}>{fmt(netWorth)}</div></Card>
      </div>
        ...
        <div style={{ height: 230 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <Line type="monotone" dataKey="assets" name="Tài sản" stroke={C.violetLight} strokeWidth={2} dot={false} />
              <Line type="monotone" dataKey="liabilities" name="Nợ" stroke={C.danger} strokeWidth={2} dot={false} />
              <Line type="monotone" dataKey="netWorth" name="Tài sản ròng" stroke={C.gold} strokeWidth={2.5} dot={{ fill: C.gold, r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </Card>
      ...
      {/* investment performance */}
      <Card>
        <SimpleTitle title="Hiệu suất đầu tư" action={growth !== null && <span style={{ fontSize: 11, padding: "2px 9px", borderRadius: 99, background: C.gold + "22", color: C.goldBright }}>Tài sản ròng ~{growth.toFixed(1)}%/năm<InfoTag termKey="cagr" /></span>} />
      ...
      <Card>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div><div style={{ fontSize: 12, color: C.textMuted }}>Thu nhập</div><div style={{ fontSize: 18, fontWeight: 700, color: C.success }}>{fmt(monthlyIncome)}</div></div>
          <div><div style={{ fontSize: 12, color: C.textMuted }}>Chi tiêu + trả nợ</div><div style={{ fontSize: 18, fontWeight: 700, color: C.danger }}>{fmt(monthlyExpense + totalMonthlyDebtService)}</div></div>
          <div><div style={{ fontSize: 12, color: C.textMuted }}>Dòng tiền ròng</div><div style={{ fontSize: 18, fontWeight: 700, color: netCashflow >= 0 ? C.success : C.danger }}>{netCashflow >= 0 ? "+" : ""}{fmt(netCashflow)}{savingsRate !== null ? ` (${savingsRate >= 0 ? "+" : ""}${savingsRate.toFixed(0)}%)` : ""}</div></div>
        </div>
          ...
          <div className="flex flex-col gap-4">
            {goals.map((g) => { const pct = Math.min(100, ((Number(g.currentAmount) || 0) / (Number(g.targetAmount) || 1)) * 100); return (
              <div key={g.id}>
                ...
                <div style={{ height: 10, background: C.bg, borderRadius: 6, overflow: "hidden" }}><div style={{ width: `${pct}%`, height: "100%", background: `linear-gradient(90deg, ${C.violet}, ${C.gold})`, borderRadius: 6 }} /></div>
              </div>
            ); })}
          </div>
        )}
    ...
    <div className="flex flex-col gap-5">
        ...
        {assets.length === 0 ? <EmptyState text="Chưa có tài sản nào." /> : (
          <div className="flex flex-col gap-2">
            {assets.map((a) => {
              ...
              return (
                <div key={a.id} className="flex items-center justify-between rounded-xl px-4 py-3" style={{ background: C.bg, border: `1px solid ${C.border}` }}>
                  <div className="flex items-center gap-3">
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: C.bgPanel2, display: "flex", alignItems: "center", justifyContent: "center" }}><Icon size={16} color={tier.color} /></div>
                    <div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right"><div style={{ fontSize: 14, fontWeight: 600 }}>{fmtFull(a.value)}</div>{a.costBasis ? <div style={{ fontSize: 11, color: gain >= 0 ? C.success : C.danger }}>{gain >= 0 ? "+" : ""}{fmt(gain)}</div> : null}</div>
                    {a.source !== "synced" && <button onClick={() => setModal({ type: "asset", editing: a })} style={{ color: C.textMuted }}><Pencil size={14} /></button>}
      ...
      <Card>
        ...
        {liabilities.length === 0 ? <EmptyState text="Chưa có khoản vay nào." /> : (
          <div className="flex flex-col gap-2">
            {liabilities.map((l) => (
              <div key={l.id} className="flex items-center justify-between rounded-xl px-4 py-3" style={{ background: C.bg, border: `1px solid ${C.border}` }}>
                <div><div style={{ fontSize: 14, fontWeight: 500 }}>{l.name}</div><div style={{ fontSize: 11, color: C.textFaint }}>Lãi suất {l.interestRate || 0}%/năm{l.dueDate ? ` · đáo hạn ${l.dueDate}` : ""}{liabilityMonthlyPayment(l) > 0 ? ` · trả góp ${fmtFull(liabilityMonthlyPayment(l))}/th` : ""}</div></div>
                <div className="flex items-center gap-4">
        ...
        {accounts.length === 0 ? <EmptyState text="Chưa kết nối tài khoản nào." /> : (
          <div className="flex flex-col gap-2">
            {accounts.map((a) => (
              <div key={a.id} className="flex items-center justify-between rounded-xl px-4 py-3" style={{ background: C.bg, border: `1px solid ${C.border}` }}>
                <div className="flex items-center gap-3">
                  <div style={{ width: 34, height: 34, borderRadius: 10, background: C.bgPanel2, display: "flex", alignItems: "center", justifyContent: "center" }}><Landmark size={15} color={C.gold} /></div>
                  <div><div style={{ fontSize: 14, fontWeight: 500 }}>{a.name}</div><div style={{ fontSize: 11, color: C.textFaint }}>Đồng bộ lần cuối: {a.lastSynced} · <span style={{ color: C.success }}>Đã kết nối</span></div></div>
                </div>
                <button onClick={() => onSync(a.id)} style={{ color: C.textMuted }}><RefreshCw size={15} /></button>
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <ModalShell title={editing ? "Sửa khoản vay" : "Thêm khoản vay"} onClose={onClose}>
      <div className="flex flex-col gap-3">
        ...
        {computedPayment > 0 && <div style={{ fontSize: 12, color: C.goldBright }}>Ước tính trả góp: {fmtFull(computedPayment)}/tháng</div>}
  ...
  return (
    <div className="flex flex-col gap-5">
      <Card>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div><div style={{ fontSize: 12, color: C.textMuted }}>Dòng tiền khả dụng/tháng</div><div style={{ fontSize: 18, fontWeight: 700 }}>{fmt(netCashflow)}</div></div>
          <div><div style={{ fontSize: 12, color: C.textMuted }}>Tổng cam kết cho mục tiêu</div><div style={{ fontSize: 18, fontWeight: 700, color: overCommitted ? C.danger : C.text }}>{fmt(totalContribution)}</div></div>
          <div><div style={{ fontSize: 12, color: C.textMuted }}>Tăng trưởng tài sản ròng gần đây</div><div style={{ fontSize: 18, fontWeight: 700 }}>{growth !== null ? `~${growth.toFixed(1)}%/năm` : "—"}</div></div>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {GOAL_TEMPLATES.map((t) => (
            <button key={t.key} onClick={() => { setTemplate(t); setEditing(null); setModal(true); }} className="flex items-center gap-1.5 px-3.5 py-2 rounded-full text-sm" style={{ background: C.bg, border: `1px solid ${C.borderLight}`, color: C.textMuted }}>
              <t.icon size={13} color={C.gold} /> {t.name}
            </button>
          ))}
        </div>
      </Card>
...
function GoalCard({ g, growth, onEdit, onRemove }) {
  ...
  return (
    <div className="rounded-xl p-4" style={{ background: C.bg, border: `1px solid ${C.border}` }}>
      ...
      <div style={{ height: 8, background: C.bgPanel2, borderRadius: 5, overflow: "hidden", marginBottom: 8 }}><div style={{ width: `${pct}%`, height: "100%", background: onTrack ? C.success : C.gold, borderRadius: 5 }} /></div>
      ...
      <div style={{ fontSize: 11, color: onTrack ? C.success : C.gold, marginTop: 6 }}>{onTrack ? "Đang trên đà đạt mục tiêu" : "Có thể cần tăng đóng góp hoặc điều chỉnh thời hạn"}</div>
      {growth !== null && (
...
function AdvisorTab({ messages, setMessages, snapshot, bookings, setBookings, recommendations }) {
  ...
  const buildSystemPrompt = () => {
    const allocText = snapshot.allocation.map((a) => `${a.name}: ${fmtFull(a.value)}`).join("; ");
    const liabText = snapshot.liabilities.map((l) => `${l.name}: dư nợ ${fmtFull(l.balance)}, lãi suất ${l.interestRate || 0}%/năm`).join("; ") || "Không có";
    const goalsText = snapshot.goals.map((g) => `${g.name}: mục tiêu ${fmtFull(g.targetAmount)} vào ${g.targetDate}`).join("; ") || "Chưa đặt mục tiêu";
    return `Bạn là cố vấn gia sản AI, trả lời tiếng Việt, giọng THẬN TRỌNG. Chỉ trình bày dữ kiện và kịch bản rủi ro/cơ hội, KHÔNG khuyến nghị dứt khoát. Không bịa số liệu thị trường. Bối cảnh: tổng tài sản ${fmtFull(snapshot.totalAssets)}, tổng nợ ${fmtFull(snapshot.totalLiabilities)}, tài sản ròng ${fmtFull(snapshot.netWorth)}. Phân bổ: ${allocText || "chưa có"}. Khoản vay: ${liabText}. Thu nhập ~${fmtFull(snapshot.monthlyIncome)}/tháng, chi tiêu ~${fmtFull(snapshot.monthlyExpense)}/tháng. Mục tiêu: ${goalsText}.`;
  };
  const send = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 1000, system: buildSystemPrompt(), messages: newMessages.map((m) => ({ role: m.role, content: m.content })) }) });
      const data = await res.json();
      const text = (data.content || []).filter((b) => b.type === "text").map((b) => b.text).join("\n") || "Chưa nhận được phản hồi, thử lại nhé.";
        ...
        <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 12, paddingRight: 4 }}>
          ...
          {messages.map((m, i) => (
            <div key={i} style={{ alignSelf: m.role === "user" ? "flex-end" : "flex-start", maxWidth: "85%" }}>
              <div style={{ background: m.role === "user" ? `linear-gradient(135deg, ${C.violetDeep}, #3A2C5C)` : C.bg, border: `1px solid ${m.role === "user" ? C.violet + "55" : C.border}`, borderRadius: 14, padding: "10px 14px", fontSize: 13.5, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{m.content}</div>
            </div>
          ))}
      ...
      <Card>
        ...
        <div className="flex flex-col gap-2.5">
          {sortedAdvisors.map((a) => { const rec = recommendations[a.id]; return (
            <div key={a.id} className="flex items-center justify-between rounded-xl px-4 py-3" style={{ background: C.bg, border: `1px solid ${rec ? C.gold + "55" : C.border}` }}>
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span style={{ fontSize: 14, fontWeight: 600 }}>{a.name}</span>
                  <span style={{ fontSize: 10, padding: "1px 7px", borderRadius: 99, background: C.violetDeep, color: C.violetLight }}>{a.level}</span>
                  {rec && <span style={{ fontSize: 10, padding: "1px 7px", borderRadius: 99, background: C.gold + "22", color: C.goldBright }}>Đề xuất cho bạn</span>}
                </div>
                <div style={{ fontSize: 12, color: C.textMuted }}>{a.title}</div>
                <div style={{ fontSize: 11, color: C.textFaint }}>{a.specialties.join(" · ")}</div>
                {rec && <div style={{ fontSize: 11, color: C.gold, marginTop: 2 }}>{rec}</div>}
                <div className="flex items-center gap-1 mt-1" style={{ fontSize: 11, color: C.gold }}><Star size={11} fill={C.gold} />{a.rating} ({a.reviews} đánh giá)</div>
              </div>
              <div className="text-right flex flex-col items-end gap-2">
                <div style={{ fontSize: 14, fontWeight: 700, color: C.goldBright }}>{fmt(a.rate)}/giờ</div>
      ...
      <Card>
        ...
        <div className="flex gap-2 flex-wrap">
          {[{ id: "purchase", label: "Mua tài sản mới bằng vay" }, { id: "stress", label: "Stress-test lãi suất" }, { id: "prepay", label: "Trả nợ sớm" }].map((s) => (
            <button key={s.id} onClick={() => setScenario(s.id)} className="px-3.5 py-2 rounded-full text-sm" style={{ background: scenario === s.id ? `linear-gradient(135deg, ${C.violet}, ${C.violetDeep})` : "transparent", border: `1px solid ${scenario === s.id ? C.violet : C.borderLight}`, color: scenario === s.id ? "#fff" : C.textMuted }}>{s.label}</button>
          ))}
        </div>
      </Card>
...
function AIInterpretButton({ getPayload }) {
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 800, system: `Bạn là cố vấn gia sản, giọng THẬN TRỌNG. Chỉ diễn giải bộ số liệu ĐÃ TÍNH SẴN được cung cấp (bao gồm cả bối cảnh mục tiêu tài chính và phân bổ tài sản hiện tại nếu có) thành nhận định rủi ro/cơ hội bằng tiếng Việt, gạch đầu dòng, ngắn gọn, có liên hệ tới mục tiêu/danh mục hiện tại nếu dữ liệu cho phép. KHÔNG bịa số liệu. KHÔNG khuyến nghị dứt khoát.`, messages: [{ role: "user", content: `Dữ liệu kịch bản (JSON):\n${JSON.stringify(getPayload(), null, 2)}\n\nDiễn giải rủi ro và cơ hội, có liên hệ tới mục tiêu/danh mục nếu phù hợp.` }] }) });
      const data = await res.json();
      setText((data.content || []).filter((b) => b.type === "text").map((b) => b.text).join("\n") || "Không nhận được phản hồi.");
    } catch (e) { setText("Có lỗi khi kết nối AI."); }
    setLoading(false);
  };
  return (
    <div className="mt-2">
      <GhostButton onClick={run} style={{ borderColor: C.violet, color: C.violetLight }}>{loading ? <Loader2 size={14} className="animate-spin" /> : <MessageCircle size={14} />} Nhờ AI diễn giải kết quả này</GhostButton>
      {text && <div className="mt-3 rounded-xl p-3.5" style={{ background: C.bg, border: `1px solid ${C.violet}44`, fontSize: 13, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{text}</div>}
    </div>
  );
}
        ...
        <>
          <div className="rounded-xl p-4 mb-3" style={{ background: C.bg, border: `1px solid ${C.border}` }}>
            <div className="grid grid-cols-2 gap-4 mb-3">
              <div><div style={{ fontSize: 11, color: C.textFaint }}>Số tiền vay</div><div style={{ fontSize: 17, fontWeight: 700, color: C.goldBright }}>{fmtFull(loanAmt)}</div></div>
              <div><div style={{ fontSize: 11, color: C.textFaint }}>Trả góp / tháng</div><div style={{ fontSize: 17, fontWeight: 700, color: C.goldBright }}>{fmtFull(newPayment)}</div></div>
            </div>
          ...
          <div className="rounded-xl p-4 mb-3" style={{ background: C.bg, border: `1px solid ${C.violet}33` }}>
            <div style={{ fontSize: 11, color: C.violetLight, marginBottom: 6 }}>Chuẩn tài chính: giá trị hiện tại (PV) & tương lai (FV)<InfoTag termKey="pv_fv" /><InfoTag termKey="npv" /></div>
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div><span style={{ color: C.textMuted }}>Quy về hiện giá (NPV, chiết khấu {discountRate}%/năm)</span><div style={{ fontSize: 14, fontWeight: 600, color: C.goldBright }}>{fmt(npvLoanCost)}</div></div>
              ...
              <div><span style={{ color: C.textMuted }}>Chi phí cơ hội (FV nếu đầu tư thay vì trả trước)</span><div style={{ fontSize: 14, fontWeight: 600, color: C.gold }}>{fmt(downPaymentFV)}</div></div>
            </div>
          </div>
          ...
          {!l?.termMonths ? <div style={{ color: C.textFaint, fontSize: 13 }}>Khoản vay này chưa có "kỳ hạn còn lại".</div> : amt > 0 ? (
            <div className="rounded-xl p-4" style={{ background: C.bg, border: `1px solid ${C.border}` }}>
              <div className="grid grid-cols-2 gap-4 mb-2">
                ...
                <div><div style={{ fontSize: 11, color: C.textFaint }}>Lãi tiết kiệm ước tính</div><div style={{ fontSize: 17, fontWeight: 700, color: C.goldBright }}>{fmtFull(Math.max(0, interestSaved))}</div></div>
              </div>
              ...
              <div className="rounded-xl p-3 mb-2" style={{ background: C.bgPanel2, border: `1px solid ${C.violet}33` }}>
                <div className="flex items-center justify-between mb-2">
                  <div style={{ fontSize: 11, color: C.violetLight }}>So sánh: trả nợ vs đầu tư số tiền này</div>
                  <div className="flex items-center gap-1"><span style={{ fontSize: 11, color: C.textFaint }}>Lợi nhuận thay thế</span><TextInput type="number" style={{ width: 55 }} value={altReturn} onChange={(e) => setAltReturn(e.target.value)} /><span style={{ fontSize: 11, color: C.textFaint }}>%/năm</span></div>
                </div>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  ...
                  <div><span style={{ color: C.textMuted }}>Lãi ước tính nếu đầu tư thay vào đó ({remainingYears.toFixed(0)} năm)</span><div style={{ fontSize: 14, fontWeight: 600, color: C.gold }}>{fmt(investGain)}</div></div>
                </div>
              </div>
...
function RuleRow({ enabled, onToggle, label, children }) {
  return (
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2.5"><button onClick={onToggle} style={{ color: enabled ? C.gold : C.textFaint }}>{enabled ? <Bell size={16} /> : <BellOff size={16} />}</button><span style={{ fontSize: 13, color: enabled ? C.text : C.textFaint }}>{label}</span></div>
      {children}
    </div>
  );
}
